...
void spi_init(void){
  ...
  SPCR  |=   0b11010000; //enable SPI, master mode 
  SPSR  |=   0b00000001; // double speed operation, interrup disabled,
 }//spi_init
...
ISR(TIMER0_OVF_vect){
  ...
  if ((count_7ms % 64)==0){ //?? interrupts equals one half second 
    ...
    while(!(SPSR & (1<<SPIF))); //wait till data sent out (while loop)
  ...
  if (display_count == 0){display_count=1;} //back to 1st positon
}
