...
void increment_timer() {
	...
	switch(timer_ticks){
		...
		case 130: time_array_secminhr[0]++; 
			  ...
			  timer_ticks = 0;
			  if (ALM[0] == 1 && ALM[1] == time_array_secminhr[1] && ALM[2] == time_array_secminhr[2]) {
				  uint8_t temp = PORTB;
...
void alarm_routine(uint8_t option) {
	switch(option) {
	//turn off
		case 0: alarm_ringing = 0; ALM[0] = 0; break;
	//turn on
...
ISR(TIMER0_COMP_vect){
	...
	if (ALM[0] == 0 && (add_mode & 0x01)) 
		string2lcd("ARMED");
	//disabled armed message on the transition between it being armed and ALM[0] being set to 0
	else if (ALM[0] == 1 && !(add_mode & 0x01))
		clear_display();
	ALM[0] = (add_mode & 0x01); 
	if (alarm_ringing == 1 && ALM[0] == 0) //end alarm tone if alarm disable is pressed
		alarm_routine(0);
